...
if (Test-Path $srcDir) {
    $destSrcDir = Join-Path $INSTALL_DIR "src"
    if (Test-Path $destSrcDir) {
        Remove-Item -Recurse -Force $destSrcDir
    }
    Copy-Item -Recurse $srcDir $destSrcDir
    Write-Host "✅ 已复制 src/ 目录" -ForegroundColor Green
} else {
...
foreach ($configFile in $CONFIG_FILES) {
    $sourcePath = Join-Path $SCRIPT_DIR $configFile
    if (Test-Path $sourcePath) {
        Copy-Item $sourcePath $INSTALL_DIR -Force
    } else {
        Write-Host "⚠️  警告: 找不到 $configFile" -ForegroundColor Yellow
    }
}
