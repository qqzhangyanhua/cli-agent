...
class GitTools:
    ...
    def analyze_changes(self) -> Dict:
        ...
        if staged_diff.get("has_diff"):
            summary_parts.append(f"已暂存 {len(staged_diff.get('files_changed', []))} 个文件")
        if unstaged_diff.get("has_diff"):
            summary_parts.append(f"未暂存 {len(unstaged_diff.get('files_changed', []))} 个文件")
